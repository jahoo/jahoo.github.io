...
export function drawYAxis(ctx, L) {
...
export function drawLeftHistogram(ctx, L, values, opts) {
    opts = opts || {};
    if (!values) return;
...
export function drawRightCDF(ctx, L, probeList, probeColor, opts) {
    opts = opts || {};
...
export function drawStaircase(ctx, L, cs, color, lineWidth, dash, alpha) {
...
export function drawProbe(ctx, L, cs, u, color, alpha) {
...
export function drawPanel(canvas, opts) {
...
export function drawMethodSection(canvas, sec, probeColor, opts, weights) {
    ...
    if (sec.mode === 'single' && sec.probes && sec.probes.length > 0) {
        ...
        var liveCounts = countIndices(
            sec.probes.map(function (p) { return searchSorted(cs, p.u); }), N
        );
...
export function drawResidualSection(cvSec6, sec6, weights) {
    var histValues = weights.slice();
    var histMax = Math.max.apply(null, weights) * 1.15;
    var overlay = {};
    ...
    if (sec6.mode === 'single' && sec6.residualProbes && sec6.residualProbes.length > 0) {
        ...
        var resCs = cumulativeSum(normRes2); resCs[N - 1] = 1.0;
        var stoCounts = countIndices(
            sec6.residualProbes.map(function (p) { return searchSorted(resCs, p.u); }), N
        );
    ...
    var residualPhase2 = getResidualPhase2();
    var showStrata = (residualPhase2 === 'stratified' || residualPhase2 === 'systematic') && R > 0;
    ...
    var panelOpts = {
        ...
        comb: residualPhase2 === 'systematic' && R > 0,
    };
    for (var key in overlay) panelOpts[key] = overlay[key];
...
export function drawBranchKillSection(cvBK, secBK, weights) {
    if (!cvBK) return;

    var result = resetCanvas(cvBK);
    var ctx = result.ctx, w = result.w, h = result.h;
    var L = panelLayout(w, h);
    ...
    var histOpts = { maxVal: histMax, detCopies: detCopies, methodColor: METHOD_COLORS.branchkill };
    ...
    for (var i = 0; i < N; i++) {
        ...
        var pi = N * weights[i] - detCopies[i];
        var oneMinusP = 1 - pi;
...
export function drawEstimatorDist(canvas, datasets, trueVal) {
    if (!datasets || datasets.every(function (d) { return !d.estimators; })) { resetCanvas(canvas); return; }
    var result = resetCanvas(canvas);
    var ctx = result.ctx, w = result.w, h = result.h;
    ...
    for (var ci = 0; ci < nDS; ci++) {
        ...
        ctx.beginPath();
        ctx.moveTo(loX, eby); ctx.lineTo(hiX, eby);
        ctx.moveTo(loX, eby - 3); ctx.lineTo(loX, eby + 3);
        ctx.moveTo(hiX, eby - 3); ctx.lineTo(hiX, eby + 3);
        ctx.stroke();
        // Mean dot
...
export function drawComparisonPanel(weights, compData) {
    ...
    var methods = [
        { hist: compData.multi, color: METHOD_COLORS.multinomial },
        { hist: compData.strat, color: METHOD_COLORS.stratified },
        { hist: compData.sys,   color: METHOD_COLORS.systematic },
        { hist: compData.resid, color: METHOD_COLORS.residual },
    ];
...
export function drawEstDist(canvasId, sec, color, label, weights) {
    var cv = document.getElementById(canvasId);
    if (!cv) return;
    // Skip if canvas is hidden (zero size) -- will be drawn on reveal
    if (cv.offsetWidth === 0 || cv.offsetHeight === 0) return;
    var hist = sec.hist;
    if (!hist || !hist.allCounts) { resetCanvas(cv); return; }
    // Recompute estimator values for current test function (cheap)
    var ev = evalEstimators(hist, weights);
    if (!ev) { resetCanvas(cv); return; }
...
export function drawCompEstDist(compData, weights) {
    var cv = document.getElementById('cv-est-all');
    if (!cv || !compData) { if (cv) resetCanvas(cv); return; }
    // Recompute all estimator values for current test function
    var evM = evalEstimators(compData.multi, weights);
    var evS = evalEstimators(compData.strat, weights);
    var evY = evalEstimators(compData.sys, weights);
    var evR = evalEstimators(compData.resid, weights);
    if (!evM) { resetCanvas(cv); return; }
    var residualPhase2 = getResidualPhase2();
    drawEstimatorDist(cv, [
        ...
        { estimators: evR.estimators, color: METHOD_COLORS.residual,   label: 'Resid-' + ({multinomial:'Multi',stratified:'Strat',systematic:'Syst'}[residualPhase2] || 'Multi') },
    ], evM.trueVal);
}

// countIndices re-exported for use in main.js (avoids circular import)
export { countIndices } from './algorithms.js';
