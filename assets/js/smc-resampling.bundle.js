(() => {
  ...
  var TESTFN_OPTIONS = [
    ...
    { value: "square", text: "(i/N)\xB2", math: "\\((\\idx/\\np)^2\\)" },
  ...
  function createDropdown(opts) {
    ...
    function findSelected() {
      ...
      return opts.items.find(function(it) {
        return it.value === k;
      }) || null;
    }
  ...
  function initToolbar(opts) {
    ...
    var weightsMatchPreset = function(weights2) {
      for (var k = 0; k < PRESET_ORDER.length; k++) {
        var p = PRESETS[PRESET_ORDER[k]]();
        if (p.length !== weights2.length) continue;
        var match = true;
        for (var i = 0; i < p.length; i++) {
          if (Math.abs(p[i] - weights2[i]) > 1e-9) {
            match = false;
            break;
          }
        }
    ...
    if (weightsBtn && weightsMenu) {
      weightsDropdown = createDropdown({
        btn: weightsBtn,
        menu: weightsMenu,
        items: PRESET_ORDER.map(function(k) {
          return { value: k, label: PRESET_LABELS[k] };
        }),
        getSelectedKey: function() {
          return weightsMatchPreset(getWeights2());
        },
        ...
        renderTrigger: function(btn, item) {
          ...
          drawMiniWeights(cv, getWeights2(), 36, 24);
          btn.appendChild(cv);
    ...
    setInterval(function() {
      if (weightsDropdown) {
        var json = JSON.stringify(getWeights2());
